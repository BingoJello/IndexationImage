...
END LOOP;
                IF y = nearests_img.COUNT THEN
                     min_score_contour := score_contour;
                     similar_img_name := line.name;
END IF;
END IF;
END LOOP;
        IF min_score_contour != MIN_VALUE THEN
            nearests_img.EXTEND(1);
            nearests_img_score.EXTEND(1);
            nearests_img(index_tab) := similar_img_name;
            nearests_img_score(index_tab) := min_score_contour;
            index_tab := index_tab + 1;
END IF;
        IF nearests_img.COUNT >= nbr_nearest OR min_score_contour = MIN_VALUE THEN
            x := 0;
END IF;
END LOOP;

    DBMS_OUTPUT.PUT_LINE('Calcul des 10 images les plus proches avec les contours pixels');
...
ELSE
        ...
        DBMS_OUTPUT.PUT_LINE('-----------------------');
FOR i in 1..nearests_img.COUNT LOOP
            DBMS_OUTPUT.PUT_LINE(nearests_img(i) || ' avec différence contours pixels de : ' || nearests_img_score(i));
END LOOP;
END IF;
END;